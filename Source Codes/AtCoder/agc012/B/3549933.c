...
void rep(int ver,int dis,int col){
  if(fl[ver]>=dis){return;}
  if(res[ver]==0){res[ver]=col;}
  fl[ver]=dis;
  int i;
  for(i=x[ver].st;i<x[ver].st+x[ver].kz;i++){
    rep(g[i].fi,dis-1,col);
  }
}

int main(void){
    int i,j,n,m,k,a,b,h,w,r=0,l,t,q;
    int v[524288],d[524288],c[524288];
    char s[524288];
    scanf("%d%d",&n,&m);
    for(i=0;i<=n;i++){
      fl[i]=-1;
