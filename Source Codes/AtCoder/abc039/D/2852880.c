...
int main(void){
    int fl,i,j,n,m,k,a[524288],b,c,h,w,r=0,l;
    double d;
    char s[128][128]={0},t[128][128]={0},u[128][128]={0};
    scanf("%d%d",&h,&w);
    //l=strlen(s);
    for(i=0;i<h;i++){scanf("%s",s[i]);
        for(j=0;j<w;j++){t[i][j]='.';u[i][j]='.';}
    }
    for(i=0;i<h;i++){
        for(j=0;j<w;j++){
            if(s[i][j]=='.'){continue;}
            fl=1;
            for(k=0;k<8;k++){
                if(i+dx8[k]<0 || h<=i+dx8[k] ||  j+dy8[k]<0 || w<=j+dy8[k]){continue;}
                if(s[i+dx8[k]][j+dy8[k]]=='.'){fl=0;break;}
            }
            if(fl){t[i][j]='#';}
        }
    }
    ...
    for(i=0;i<h;i++){
        if(strcmp(s[i],u[i])!=0){printf("impossible\n");return 0;}
    }
    printf("possible\n");
