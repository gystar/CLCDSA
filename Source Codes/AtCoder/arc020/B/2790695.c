...
ull solve () {
	...
	ull ev1 = 0, od1 = 0;
	for (i = 0; i < 10; i++) {
		if (cols[0][i] > cols[0][ev1]) ev1 = i;
		if (cols[1][i] > cols[1][od1]) od1 = i;
	}
	ull ev2 = (ev1 + 1) % 10;
	ull od2 = (od1 + 1) % 10;
	for (i = 0; i < 10; i++) {
		if (i != ev1 && cols[0][i] > cols[0][ev2]) ev2 = i;
		if (i != od1 && cols[1][i] > cols[1][od2]) od2 = i;
	}

	if (ev1 != od1) {
		result = n - (cols[0][ev1] + cols[1][od1]);
	} else {
		result = n - umax(cols[0][ev1] + cols[1][od2], cols[0][ev2] + cols[1][od1]);
	}
	result *= k;
