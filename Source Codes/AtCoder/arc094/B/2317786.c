...
long long msqr(long long x){
    long long st=0,fi=inf,t;
    while(st<=fi){
        t=(st+fi)/2;
        if(t*t<=x){st=t+1;}else{fi=t-1;}
    }
    return fi;
}

int main(void){
    long long i,j,n,m,k,a,b,c,w,r=0,l,t,q,bd,nbas,nr;
    double d;
    char s[262144];
    scanf("%lld",&q);
    //l=strlen(s);
    for(i=0;i<q;i++){
        scanf("%lld%lld",&a,&b);r=0;
        for(j=1;j<=10;j++){
            nbas=(a*b)-j;
            if(nbas<=0){continue;}
            bd=msqr(nbas);
            if(bd*(bd+1)>nbas){nr=2*bd-1;}else{nr=2*bd;}
            if(a<=bd){nr--;}
            if(b<=bd){nr--;}
            r=llmax(nr,r);
        }
