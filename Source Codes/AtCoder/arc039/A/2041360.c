...
ull solve () {
	...
	if (vsa < 900) {
		result = smax(result, (vsa % 100) + 900 - vsb);
	} else if (vsa < 990) {
		result = smax(result, (vsa % 10) + 990 - vsb);
	} else if (vsa < 999) {
		result = smax(result, 999 - vsb);
	}
	// printf("\t%lld\n", result);

	if (vsb >= 200) {
		result = smax(result, vsa - ((vsb % 100) + 100));
	} else if (vsb >= 110) {
		result = smax(result, vsa - ((vsb % 10) + 100));
	} else if (vsb >= 101) {
		result = smax(result, vsa - 100);
	}
