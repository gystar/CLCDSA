...
int main(void){
    I(how_island);
    I(how_line);
    long line_data[how_line][2];
    input_array2(how_line,2,line_data);
    long having_1[how_island+1];
    format_array(how_island+1,having_1,0);
    
    for(loop1=0;loop1<how_line;loop1++){
        if(line_data[loop1][0]==1){
            having_1[line_data[loop1][1]]=1;
        }
    }
    for(loop1=0;loop1<how_line;loop1++){
        if(having_1[line_data[loop1][0]] && line_data[loop1][1]==how_island){
            printf("POSSIBLE");
            END;
        }
    }
