...
int main(void){
    int i,j,n,m,k,a[131072],b[131072],c,h,w,r=0,l,t;
    bool fl[512][512]={0};
    unsigned long long cfl[512][8]={0};
    scanf("%d%d",&n,&m);
    for(i=0;i<m;i++){
      scanf("%d%d",&a[i],&b[i]);
    }
    ...
    for(i=1;i<=n;i++){
      ...
      //  printf("%d ",fl[i][j]);
      //}printf("\n");
      if(fl[i][511]==1){continue;}
      for(j=i+1;j<=n;j++){
        if(fl[j][511]==1){continue;}
        for(k=0;k<8;k++){
          if((cfl[i][k]&cfl[j][k])!=0){
            break;
          }
          if(k==7){
            //printf("%d %d\n",i,j);
