...
ull solve () {
	...
	qsort(arr, left, sizeof(hwll), phwllABcomp);
	qsort(arr + left, n - left, sizeof(hwll), phwllBAcomp);

	result = sum = 0;
	for (i = 0; i < left; i++) {
		result = smax(result, sum + arr[i].a);
		sum += (arr[i].a - arr[i].b);
	}
	for (i = n - 1; i > right; i--) {
		result = smax(result, sum + arr[i].a);
