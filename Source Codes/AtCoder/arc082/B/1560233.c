...
int main(void){
    I(how_data);
    long data[how_data+1];
    long data2[how_data+1];
    for(loop1=1;loop1<=how_data;loop1++){
        scanf("%ld",&data[loop1]);
        data2[loop1]=data[loop1];
    }
    //L to R
    long ans_ltr=0;
    for(loop1=1;loop1<how_data;loop1++){
        if(loop1==data[loop1]){
            i_temp=data[loop1];
            data[loop1]=data[loop1+1];
            data[loop1+1]=i_temp;
            ans_ltr++;
        }
    }
    if(data[how_data]==how_data){
        i_temp=data[how_data];
        data[how_data]=data[how_data-1];
        data[how_data-1]=i_temp;
        ans_ltr++;
    }
    
    //R to L
    long ans_rtl=0;
    for(loop1=how_data;loop1>=2;loop1--){
        if(loop1==data2[loop1]){
            i_temp=data2[loop1];
            data2[loop1]=data2[loop1-1];
            data2[loop1-1]=i_temp;
            ans_rtl++;
        }
    }
    if(data2[1]==1){
