...
bool dfs (ull i, ull j, sll c) {
	...
	if (i && table[i - 1][j] < c) {
		if (dfs(i - 1, j, c)) return true;
	}
	if (j && table[i][j - 1] < c) {
		if (dfs(i, j - 1, c)) return true;
	}
	if (i < h - 1 && table[i + 1][j] < c) {
		if (dfs(i + 1, j, c)) return true;
	}
	if (j < w - 1 && table[i][j + 1] < c) {
		if (dfs(i, j + 1, c)) return true;
	}
	return false;
}
...
ull solve () {
	...
	for (i = 0; i < h; i++) for (j = 0; j < w; j++) table[i][j] = -1;

	for (i = 0; i < h; i++) {
		for (j = 0; j < w; j++) {
			if (s[i][j] == 's') {
				puts(dfs(i, j, 2) ? "YES" : "NO");
			}
		}
	}
