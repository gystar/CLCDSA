...
ull solve () {
	...
	qsort(arr, n, sizeof(hwll), phwllrAcBcomp);

	i = 0;
	ki = 0;
	ull smallest = arr[0].b;
	while (i < n) {
		j = i;
		while (j < n - 1) {
			if (arr[i].a != arr[j + 1].a) break;
			j++;
		}
		if (arr[i].b < smallest) smallest = arr[i].b;
		b[smallest] += (arr[i].a - ((j < n - 1) ? arr[j + 1].a : 0)) * (j + 1);
