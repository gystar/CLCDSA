...
int fkg(char s[],int l,char res[]){
  int i,t;
  for(i=1;i<l-1;i++){
    if(s[i]=='x'){t=1;}else{t=0;}
    if(res[i]=='W'){t=1-t;}
    if(t==0){
      res[i+1]=res[i-1];
    }
    else{
      if(res[i-1]=='S'){res[i+1]='W';}
      else{res[i+1]='S';}
    }
  }
  if(s[0]=='x'){t=1;}else{t=0;}
  if(res[0]=='W'){t=1-t;}
  if(res[l-1]!=res[1] && t==0){return -1;}
  if(res[l-1]==res[1] && t==1){return -1;}
  if(s[l-1]=='x'){t=1;}else{t=0;}
  if(res[l-1]=='W'){t=1-t;}
  if(res[l-2]!=res[0] && t==0){return -1;}
  if(res[l-2]==res[0] && t==1){return -1;}
  return 0;
}

int main(void){
    int i,j,n,m,k,a,b,c,h,w,r=0,l,t;
    char s[524288],res[524288]={0};
    scanf("%d%s",&l,s);
    res[0]='S';res[1]='S';
    w=fkg(s,l,res);
    if(w==0){printf("%s\n",res);return 0;}
