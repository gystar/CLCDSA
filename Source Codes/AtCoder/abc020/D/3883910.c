...
long long tss(long long sk,long long ks,long long n){
  long long r;
  r=2*sk+ks*(n-1);
  r%=mod;
  r*=n;
...
int main(void){
    long long n,k,i,j,arr[16384],c=0;
    long long cnt[16384]={0},w,r=0,ns;
    scanf("%lld%lld",&n,&k);
    for(i=1;i*i<=k;i++){
      if(k%i==0){
        arr[c]=i;
    ...
    qsort(arr,c,sizeof(long long),llsortfnckj);
    for(i=0;i<c;i++){
      w=n/arr[i];
      ns=tss(arr[i],arr[i],w);
      for(j=0;j<i;j++){
        if(arr[j]%arr[i]==0){
          ns+=mod;
          ns-=cnt[j];
