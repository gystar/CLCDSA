...
ull solve () {
	...
	for (ki = 1; ki < h * w; ki++) {
		for (i = 0; i < h; i++) {
			for (j = 0; j < w; j++) {
				sll isblack = (s[i][j] == '#' ? 1 : 0);

				sll x = BIG;
				if (i && dp[i - 1][j][ki - 1] >= 0) {
					x = smin(x, dp[i - 1][j][ki - 1]);
				}
				if (j && dp[i][j - 1][ki - 1] >= 0) {
					x = smin(x, dp[i][j - 1][ki - 1]);
				}
				if (i < h - 1 && dp[i + 1][j][ki - 1] >= 0) {
					x = smin(x, dp[i + 1][j][ki - 1]);
				}
				if (j < w - 1 && dp[i][j + 1][ki - 1] >= 0) {
					x = smin(x, dp[i][j + 1][ki - 1]);
				}
	...
	for (i = 0; i < h * w; i++) {
		sll blacksteps = dp[goalx][goaly][i];
		if (i > k) break;
		if (blacksteps < 0) continue;

		maybe = (k - (i - blacksteps)) / blacksteps;
		// printf("%lld: %lld\n", i, blacksteps);
