...
bool fl[1024]={0};
long long res[1024],cnt=0;

void van(long long x){
  //printf("[%lld]\n",x);
  long long i,c=0;
  for(i=0;i<1024;i++){
    if(i==x){continue;}
...
int main(void){
    long long a[1024],n;
    long long i,j,m,k,b,c,h,w,rk=0,rg=0,l,t,mf=0;
    scanf("%lld",&n);
    for(i=0;i<n;i++){
      scanf("%lld",&a[i]);
      if(a[mf]<a[i]){mf=i;}
