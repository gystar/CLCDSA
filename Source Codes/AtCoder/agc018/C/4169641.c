...
int main(void){
    long long x,y,z,i,j,n,m,k,a,b,c,h,w,r=0,l,t;
    ...
    n=x+y+z;
    for(i=0;i<n;i++){
      scanf("%lld%lld%lld",&pp[i].av,&pp[i].bv,&pp[i].cv);
      pp[i].val=pp[i].av-pp[i].bv;
    }
    qsort(pp,n,sizeof(pp[0]),sdsortfnc);
    
    resheap();
    w=0;
    for(i=0;i<n-1;i++){
      w+=pp[i].cv;
      pd.cost=pp[i].av-pp[i].cv;
      w+=pd.cost;
      pqpush(pd);
      if(hsize<x){res[i]-=llinf;continue;}
      if(hsize>x){
        od=pqpop();
        w-=od.cost;
      }
    ...
    for(i=n-1;i>0;i--){
      w+=pp[i].cv;
      pd.cost=pp[i].bv-pp[i].cv;
      w+=pd.cost;
      pqpush(pd);
      if(hsize<y){res[i-1]-=llinf;continue;}
      if(hsize>y){
        od=pqpop();
        w-=od.cost;
      }
