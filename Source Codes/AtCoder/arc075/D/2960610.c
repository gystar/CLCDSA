...
void rep(long long cn,long long cp,long long cd,long long d){
    long long i,t,w,k;
    if(cp<=0){return;}
    if(cd*2>d){
        if(d%2==1){cp*=10;}
    ...
    t=p10[d-cd]-p10[cd-1];
    for(i=-20;i<=20;i++){
        if(cn+t*i>0){w=i;break;}
    }
    if(cd==1 && w<=0){k=1;}else{k=0;}
    rep(cn+t*w,cp*(10-llzt(0,w)-k),cd+1,d);
    w--;
    if(cd==1 && w<=0){k=1;}else{k=0;}
    rep(cn+t*w,cp*(10-llzt(0,w)-k),cd+1,d);
    w--;
