...
void calc(){
    long long i,t=1;
    for(i=0;i<18;i++){p10[i]=t;t*=10;}
}

void rep(long long x,long long pt,long long nd,long long dg){
    //if(x==0){printf("%lld %lld %lld\n",pt,nd,dg);}
    if(pt<=0){return;}
    long long i,t,a=0,w=0;
    if(nd>(dg/2)){
        if(dg%2==1){pt*=10;}
    ...
    t=p10[dg-nd]-p10[nd-1];
    for(i=9;i>=-15;i--){
        if(x>t*i){a=i;break;}
    }
    if(nd==1 && a>=0){w=1;}else{w=0;}
    rep(x-t*a,pt*(10-w-llzt(0,a)),nd+1,dg);
    a++;
    if(nd==1 && a>=0){w=1;}else{w=0;}
    rep(x-t*a,pt*(10-w-llzt(0,a)),nd+1,dg);
    a++;
...
int main(void){
    calc();
    long long i,j,n,m,k,a[524288],b,c,h,w,l,t;
    double d;
    char s[524288];
    scanf("%lld",&n);
    //l=strlen(s);
    for(i=2;i<=18;i++){rep(n,1,1,i);}
    //shuffledget(a,n);
