...
int main(void){
    cfact();
    long long i,a,b,c,v,q,tq=0,r=0;
    scanf("%s",s);
    l=strlen(s);
    for(i=0;i<l;i++){if(s[i]=='?'){tq++;}}
    for(i=0;i<8;i++){
      q=0;
      if((i&1<<0)!=0){t[1]='A';}else{t[1]='?';q++;}
      if((i&1<<1)!=0){t[2]='B';}else{t[2]='?';q++;}
      if((i&1<<2)!=0){t[3]='C';}else{t[3]='?';q++;}
      v=dyp();
      t[0]='*';
      //printf("%s %lld\n",t,v);
      //v*=calcnCr(tq,q);
