M=1e9+7,A[1<<17],B[1<<17],i,j,f;
main(s){
	for(gets(&f);~scanf("%d",&f);B[f]=++i){
		s=((A[i+1]=(A[i]+s)%M)-A[j=fmax(j,B[f])]+M)%M;
	}
	printf("%d\n",s);
...
./Main.c:1:27: warning: type defaults to ‘int’ in declaration of ‘i’ [-Wimplicit-int]
 M=1e9+7,A[1<<17],B[1<<17],i,j,f;
                           ^
./Main.c:1:29: warning: type defaults to ‘int’ in declaration of ‘j’ [-Wimplicit-int]
 M=1e9+7,A[1<<17],B[1<<17],i,j,f;
                             ^
./Main.c:1:31: warning: type defaults to ‘int’ in declaration of ‘f’ [-Wimplicit-int]
 M=1e9+7,A[1<<17],B[1<<17],i,j,f;
                               ^
