...
ull solve () {
	...
	for (i = 0; i < n; i++) {
		...
		ull indexshift = a[i] / 64;
		for (j = sum / 64; j >= 0; j--) {
			if (downer < 64) bitsetdp[j + indexshift + 1] |= (bitsetdp[j] >> downer);
			bitsetdp[j + indexshift] |= (bitsetdp[j] << upper);
		}
		sum += a[i];

		// printf("%lld\n\n", i);
		// for (j = 0; j < 200; j++) {
		// 	printf("  %3lld: %1d\n", j, (bitsetdp[j / 64] >> (j % 64)) & 1);
		// }
	}
	...
	result = (total + 1) / 2;
	while (true) {
		if ((bitsetdp[result / 64] >> (result % 64)) & 1) break;
		result++;
	}
