...
if(((pd*)a)->xz < ((pd*)b)->xz){return -1;}
if(((pd*)a)->xz > ((pd*)b)->xz){return 1;}
return 0;
}

int yzsortfnc(const void *a,const void *b){
if(((pd*)a)->yz < ((pd*)b)->yz){return -1;}
if(((pd*)a)->yz > ((pd*)b)->yz){return 1;}
return 0;
}
...
int main(void){
    ...
    long long sx,sy,fx,fy,xf=0,yf=0;
    long long x,y,p=0;
    pd pt[524288];
    double d;
    char s[524288];
    ...
    qsort(pt,p,sizeof(pt[0]),yzsortfnc);
    for(i=0;i<p;i++){
        pt[i].ry=i+1;
    }
    qsort(pt,p,sizeof(pt[0]),xzsortfnc);
    upd(0,0);
    for(i=0;i<p;i++){
        w=query(0,pt[i].ry,0,0,ssize);
        upd(pt[i].ry,w+1);
    }
    r=query(0,ssize,0,0,ssize);
    h=fx-sx+1;
    w=fy-sy+1;
    if(r==llmin(h,w)){
        d-=(20.0f*r);
