...
ull solve () {
	...
	for (i = 0; i < n; i++) {
		if (isinrange_soft(vua, xy[i].a, vuc) && isinrange_soft(vub, xy[i].b, vud)) {
			// hwll x = {xy[i].b, len};
			// printf("%lld,%lld!\n", xy[i].b, len);
			// sorted[len++] = x;
			a[len++] = xy[i].b;
	...
	// qsort(sorted, len, sizeof(hwll), phwllABcomp);
	...
	for (i = 0; i < len; i++) {
		...
		if (item < lis[ng]) {
			lis[ng] = item;
			lislen = umax(lislen, ng);
		}
	}

	const double quarter = M_PI * 5;

	ull original = (udiff(vua, vuc) + udiff(vub, vud));
	dresult = original * 100LL;
	if (lislen > umin(udiff(vua, vuc), udiff(vub, vud))) {
		dresult += (quarter * 2 - 20);
		lislen--;
	}
	dresult -= (20 - quarter) * (double)lislen;
