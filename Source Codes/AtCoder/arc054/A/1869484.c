...
ull solve () {
	...
	ull clockwise = ((vuc <= vud) ? (vud - vuc) : (n + vud - vuc));
	ull anticlockwise = n - clockwise;

	// printf("deb: %llu\n", clockwise);

	if (vub >= vua && anticlockwise * (vub + vua) < clockwise * (vub - vua)) {
		printquotient(anticlockwise, (vub - vua));
	} else {
		printquotient(clockwise, (vub + vua));
	}
