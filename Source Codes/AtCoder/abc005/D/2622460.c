...
ull findac (ull u, ull d, ull l, ull r) {
	return ac[d][r] - (u ? ac[u - 1][r] : 0) - (l ? ac[d][l - 1] : 0) + (u && l ? ac[u - 1][l - 1] : 0);
}
...
ull solve () {
	...
	for (i = 0; i < n; i++) {
		for (j = 0; j < n; j++) {
			ac[i][j] = (j ? ac[i][j - 1] : 0) + a[i][j];
		}
	}
	for (j = 0; j < n; j++) {
		for (i = 0; i < n; i++) {
			ac[i][j] = (i ? ac[i - 1][j] : 0) + ac[i][j];
		}
	}
	...
	for (i = 0; i < n; i++) {
		for (j = i; j < n; j++) {
			for (ki = 0; ki < n; ki++) {
				for (li = ki; li < n; li++) {
					dpcell = &b[(j - i + 1) * (li - ki + 1)];
					*dpcell = umax(*dpcell, findac(i, j, ki, li));
				}
			}
		}
	}
