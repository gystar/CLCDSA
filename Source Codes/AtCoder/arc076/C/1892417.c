...
ull solve () {
	...
	sum = 0;
	for (i = 0; i < n; i++) {
		if (isonedge(arr[i].a) && isonedge(arr[i].b)) {
			dr[sum * 2].a = converttoint(arr[i].a);
			dr[sum * 2 + 1].a = converttoint(arr[i].b);
			dr[sum * 2].b = dr[sum * 2 + 1].b = i;

			sum++;
		}
	}
	qsort(dr, sum * 2, sizeof(hwll), phwllAcomp);
...
int32_t main (void) {
	...
	// }
	for (i = 0; i < n; i++) {
		scanf("%llu%llu%llu%llu", &arr[i].a.a, &arr[i].a.b, &arr[i].b.a, &arr[i].b.b);
	}
	// for (i = 0; i < h; i++) {
	// 	for (j = 0; j < w; j++) {
	// 		scanf("%llu", &a[i][j]);
