...
int calc(int p,int q){
  if(p==0){return q;}
  if(q==b){return b+p;}
  if(p==a){return b+a+(b-q);}
  if(q==0){return b+a+b+(a-p);}
  return -1;
}

int main(void){
    int p,q,r,s;
    int i,j,n,m,k,c=0,h,w,l,t;
    bool fl[524288]={0};
    sd data[524288];
    scanf("%d%d%d",&a,&b,&n);
    for(i=0;i<n;i++){
      scanf("%d%d%d%d",&p,&q,&r,&s);
      if(calc(p,q)!=-1 && calc(r,s)!=-1){
        data[c].val=calc(p,q);
        data[c].node=i;
        c++;
        data[c].val=calc(r,s);
    ...
    if(c==0){printf("YES\n");return 0;}
    qsort(data,c,sizeof(data[0]),sdsortfnc);
    for(i=0;i<c;i++){
      if(fl[data[i].node]==0){
        fl[data[i].node]=1;
        push(data[i].node);
