...
ull solve () {
	...
	for (i = 0; i < n; i++) {
		for (j = 0; j <= k; j++) {
			for (ki = 0; ki <= k; ki++) {
				// if (s[i] != '1') {
				// 	dp[i + 1][j][ki] += dp[i][j ? j - 1 : 0][ki + 1];
				// }
				// if (s[i] != '0') {
				// 	dp[i + 1][j][ki] += dp[i][j + 1][ki ? ki - 1 : 0];
				// }
				// dp[i + 1][j][ki] %= MOD;
				if (s[i] != '1') {
					dpcell = &dp[i + 1][j + 1][ki ? ki - 1 : 0];
					*dpcell = (*dpcell + dp[i][j][ki]) % MOD;
				}
				if (s[i] != '0') {
					dpcell = &dp[i + 1][j ? j - 1 : 0][ki + 1];
					*dpcell = (*dpcell + dp[i][j][ki]) % MOD;
				}
			}
		}
