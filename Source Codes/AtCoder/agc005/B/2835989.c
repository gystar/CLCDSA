...
int main(void){
    sinit();
    long long i,j,n,m,k,a[524288],b,c,w,r=0,l,t;
    long long st,fi,te,p,q;
    double d;
    char s[524288];
    scanf("%lld",&n);
    //l=strlen(s);
    ...
    for(i=1;i<=n;i++){
        ...
        while(st<=fi){
            te=(st+fi)/2;
            w=query(te,i+1,0,0,ssize);
            if(w<a[i]){st=te+1;}else{fi=te-1;}
        }
        p=st;
        st=i;fi=n+2;
        while(st<=fi){
            te=(st+fi)/2;
            w=query(i,te,0,0,ssize);
            if(w<a[i]){fi=te-1;}else{st=te+1;}
        }
        q=fi-1;
        //printf("<%lld %lld>\n",p,q);
        r+=(a[i]*(i-p+1)*(q-i+1));
