...
ull solve () {
	...
	for (i = n; i >= 1; i--) {
		ull putindex = indexdp[i - 1];
		ull leftest, rightest;

		leftest = rightest = putindex;
		if (putindex > 0 && isput[putindex - 1]) {
			leftest = lrdp[putindex - 1].a;
		}
		if (putindex < n - 1 && isput[putindex + 1]) {
			rightest = lrdp[putindex + 1].b;
		}

		result += ((putindex - leftest + 1) * (rightest - putindex + 1)) * i;

		isput[putindex] = true;
		lrdp[leftest].a = lrdp[rightest].a = leftest;
		lrdp[leftest].b = lrdp[rightest].b = rightest;
	}
