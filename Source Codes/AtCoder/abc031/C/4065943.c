...
int main(void){
    int i,j,n,m,k,a[54],b,c,h,w,r=-inf,l,t,st,fi;
    int rw[54][2]={0};
    scanf("%d",&n);
    for(i=1;i<=n;i++){
      scanf("%d",&a[i]);
      rw[i][0]=rw[i-1][0];
      rw[i][1]=rw[i-1][1];
      if(i%2==0){rw[i][0]+=a[i];}else{rw[i][1]+=a[i];}
    }
    for(i=1;i<=n;i++){
      w=-inf;t=0;
      for(j=1;j<=n;j++){
        ...
        if(w<rw[fi][(st+1)%2]-rw[st-1][(st+1)%2]){
          w=rw[fi][(st+1)%2]-rw[st-1][(st+1)%2];
          t=j;
        }
      }
      st=min(i,t);
      fi=max(i,t);
      //printf("%d %d %d\n",i,t,rw[fi][(st)%2]-rw[st-1][(st)%2]);
      r=max(rw[fi][(st)%2]-rw[st-1][(st)%2],r);
    }
    printf("%d\n",r);
    return 0;
