...
int main(void){
    int i,j,n,m,k,a,b,c,h,w,r=0,l,fl,p;
    int sl[64],tl[64];
    int ll[16];
    double d;
    char s[64][64],t[64][64],res[16][4];
    scanf("%d%d",&k,&n);
    for(i=0;i<n;i++){
      scanf("%s%s",s[i],t[i]);
      sl[i]=strlen(s[i]);
    ...
    for(i=0;i<w;i++){
      ...
      for(j=0;j<n;j++){
        p=0;
        for(a=0;a<sl[j];a++){
          //printf("%d %d %s %d %d %s\n",j,a,res[s[j][a]-'1'],ll[s[j][a]-'1'],p,t[j]);
          if(res[s[j][a]-'1'][0]=='*'){
            for(b=0;b<ll[s[j][a]-'1'];b++){
              if(p==tl[j]){fl=1;break;}
              res[s[j][a]-'1'][b]=t[j][p];
              p++;
            }
          }
            ...
            for(b=0;b<ll[s[j][a]-'1'];b++){
              if(p==tl[j]){fl=1;break;}
              if(res[s[j][a]-'1'][b]!=t[j][p]){fl=1;break;}
              p++;
            }
          }
          if(fl==1){break;}
