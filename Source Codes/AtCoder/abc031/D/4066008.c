...
int main(void){
    int i,j,n,m,k,a,b,c,h,w,r=0,l,p,fl,tl;
    int nl[64];
    char s[64][64],t[64][64];
    char num[64][64];
    scanf("%d%d",&k,&n);
    for(i=0;i<n;i++){
      scanf("%s%s",s[i],t[i]);
    ...
    for(i=0;i<w;i++){
      ...
      for(j=0;j<n;j++){
        ...
        for(a=0;a<l;a++){
          //printf("%s *%s*\n",s[j][a],num[s[j][a]-'0']);
          if(strcmp(num[s[j][a]-'0'],"")==0){
            for(b=0;b<nl[s[j][a]-'0'];b++){
              if(p==tl){fl=1;break;}
              num[s[j][a]-'0'][b]=t[j][p];
              p++;
            }
            ...
            for(b=0;b<nl[s[j][a]-'0'];b++){
              if(p==tl){fl=1;break;}
              if(num[s[j][a]-'0'][b]!=t[j][p]){fl=1;break;}
              p++;
            }
          }
        }
