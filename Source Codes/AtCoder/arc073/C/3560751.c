...
int main(void){
    ...
    long long st,fi;
    long long fl[524288]={0},cnt=0;
    sd ss[524288],ls[524288],as[524288];
    double d;
    char s[524288];
    scanf("%lld",&n);
    for(i=0;i<n;i++){
    ...
    qsort(ss,n,sizeof(ss[0]),sdsortfnc);
    qsort(ls,n,sizeof(ls[0]),sdsortfnc);
    qsort(as,2*n,sizeof(as[0]),sdsortfnc);
    r=(ls[n-1].val-ls[0].val)*(ss[n-1].val-ss[0].val);
    st=1;fi=1;
    k=as[2*n-1].val-as[0].val;
    while(1){
      if(cnt>=n){
        r=llmin(r,k*(as[fi-1].val-as[st].val));
        if(fl[as[st].node]==1){
          cnt--;
        }
        fl[as[st].node]--;
