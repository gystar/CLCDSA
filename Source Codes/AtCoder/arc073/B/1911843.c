...
ull solve () {
	...
	for (i = 0; i < n; i++) {
		ull costdiff = b[i] - b[0];
		wall[costdiff][dp[costdiff]] = a[i];
		dp[costdiff]++;
	}

	for (i = 0; i < 4; i++) {
		qsort(wall[i], dp[i], sizeof(ull), pullcomp);
		for (j = 0; j <= dp[i]; j++) {
			dq[i][j] = (j ? dq[i][j - 1] + wall[i][(dp[i] - 1) - (j - 1)] : 0);
		}
	}
	...
	for (i = 0; i <= dp[1]; i++) {
		...
		for (j = 0; j <= dp[2]; j++) {
			if (i * (b[0] + 1) + j * (b[0] + 2) > k) break;

			for (ki = 0; ki <= dp[3]; ki++) {
				ull cost = i * (b[0] + 1) + j * (b[0] + 2) + ki * (b[0] + 3);
				if (cost > k) break;

				ull value = dq[1][i] + dq[2][j] + dq[3][ki] + dq[0][umin((k - cost) / b[0], dp[0])];
