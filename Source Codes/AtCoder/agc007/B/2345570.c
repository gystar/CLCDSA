...
int main(void){
  int n,p[32768],i,a[32768],b[32768],ip[32768];
  scanf("%d",&n);
  for(i=0;i<n;i++){
    scanf("%d",&p[i]);ip[p[i]-1]=i;
    a[i]=30000*i;b[i]=30000*30000-a[i];
  }
  for(i=0;i<n;i++){a[i]+=ip[i];}
  for(i=0;i<n;i++){if(i){printf(" ");}printf("%d",a[i]+1);}printf("\n");
  for(i=0;i<n;i++){if(i){printf(" ");}printf("%d",b[i]+1);}printf("\n");
