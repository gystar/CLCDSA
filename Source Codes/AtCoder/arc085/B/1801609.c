...
int main(void){
    int i,n,z,w,a[2048],hab[2048],lab[2048],r,tb=0,nf=0,iu,iuw;
    scanf("%d%d%d",&n,&z,&w);
    for(i=1;i<=n;i++){scanf("%d",&a[i]);}
    hab[n]=-inf;
    for(i=n-1;i>=1;i--){
        hab[i]=max(hab[i+1],zt(a[n],a[i]));
    }
    lab[n]=inf;
    for(i=n-1;i>=1;i--){
        lab[i]=min(lab[i+1],zt(a[n],a[i]));
    }
    while(nf<n){
        if(tb==0){
            iu=n;
