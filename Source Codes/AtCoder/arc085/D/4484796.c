...
ull solve () {
	...
	for (i = 1; i < base * 2; i++) seg[i] = VERYBIG;

	cnt0[0] = 0;
	for (i = 0; i < n; i++) {
		cnt0[i + 1] = cnt0[i] + (1 - d[i]);
	}
	for (i = 0; i <= n; i++) {
		dp0[i] = dp1[i] = VERYBIG;
	}
	...
	j = 0;
	for (i = 1; i <= n; i++) {
		dp0[i] = smin(dp0[i - 1], dp1[i - 1]) + d[i - 1];
		while (j < q && xy[j].a == i) {
			ull r = xy[j].b;
			// printf("%lld-%lld ?\n", i, r);
			set1(r, dp0[i - 1] + (cnt0[r] - cnt0[i - 1]));
			set1(r, find(i - 1, r) - (cnt0[n] - cnt0[r]));
			j++;
		}
		// printf("%lld, %lld\n", dp0[i], dp1[i]);
	}
	result = smin(dp0[n], dp1[n]);
