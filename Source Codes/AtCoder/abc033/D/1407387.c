...
void sort(datatype *origin, int left, int right){
	static position tmp[2000];
	if(right - left > 1){
		int i, j, k, half = (left + right) / 2;
		sort(origin, left, half);
		sort(origin, half, right);
		for(i = left; i < right; i++){
			tmp[i] = origin[i];
		}
		for(i = left, j = left, k = half; i < right; i++){
			if((tmp[j].y * tmp[k].x <= tmp[j].x * tmp[k].y && j < half) || k == right){
				origin[i] = tmp[j];
				j++;
			}
			else{
...
signed main(){
	...
	int acute_ans = 0, right_ans = 0, obtuse_ans = 0, pairnum = ((N - 1) * (N - 2)) / 2;
