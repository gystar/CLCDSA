...
ull solve () {
	...
	ull x[] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
	const char formatstr[] = "%04llu/%02llu/%02llu\n";

	scanf("%llu/%llu/%llu", &vua, &vub, &vuc);
	if (vua % 4 == 0) {
		if (vua % 400 == 0 || vua % 100) {
			x[1] = 29;
		}
	}
	...
	for (i = vub; i <= 12; i++) {
		if (vua % i == 0) {
			if (i == vub) {
				for (j = vuc; j <= x[i - 1]; j++) {
					if ((vua / i) % j == 0) {
						printf(formatstr, vua, vub, j);
						return 0;
					}
				}
			} else {
				printf(formatstr, vua, i, 1ULL);
				return 0;
			}
		}
	}
	printf(formatstr, vua + 1, 1ULL, 1ULL);
