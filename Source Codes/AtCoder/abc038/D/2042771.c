...
ull solve () {
	...
	while (i < n) {
		...
		while (arr[i].a == arr[j].a) {
			k = arr[j].b;
			sll leftlength = nibutanlobo(func, 0, n + 1) + 1;

			if (leftlength >= 0) {

				if (newlis[leftlength]) {
					newlis[leftlength] = umin(newlis[leftlength], k);
				} else {
					changedindex[differs++] = leftlength;
					newlis[leftlength] = k;
				}
			}
		...
		for (i = 0; i < differs; i++) {
			ull ci = changedindex[i];
			lis[ci] = umin(lis[ci], newlis[ci]);
			newlis[ci] = 0;
		}
	...
	k = BIG;
	result = nibutanlobo(func, 0, n + 1) + 1;
