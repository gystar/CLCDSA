...
int main(void){
    sinit();
    sd dat[524288];
    long long i,j,n,h[524288],a[524288],w;
    scanf("%lld",&n);
    for(i=0;i<n;i++){
      scanf("%lld%lld",&dat[i].av,&dat[i].bv);
    }
    qsort(dat,n,sizeof(dat[0]),sdsortfnc);
    sprquery(0,ssize,0,0,0,ssize);
    for(i=0;i<n;i++){
        w=maxquery(dat[i].bv,dat[i].bv+1,0,0,ssize);
        w++;
        sprquery(0,dat[i].bv,w,0,0,ssize);
    }
    printf("%lld\n",maxquery(0,ssize,0,0,ssize));
    return 0;
