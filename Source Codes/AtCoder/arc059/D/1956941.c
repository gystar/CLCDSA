...
ull solve () {
	...
	for (i = 1; i <= n; i++) {
		for (j = 0; j <= i; j++) {
			dpcell = &dp[i][j];
			if (j < i) *dpcell = dp[i - 1][j + 1]; // B
			if (j) {
				*dpcell += dp[i - 1][j - 1] * 2; // 0,1
			} else {
				*dpcell += dp[i - 1][0]; // B
			}
			*dpcell %= MOD;
	...
	// printf("(%llu) ", dp[n][slen]);

	ull division = bitpow(bitpow(2, slen, MOD), (MOD - 2), MOD);
	// division = bitpow(division, (MOD - 2), MOD);
	// printf("(divis: %llu) ", division);
	result = dp[n][slen] * division % MOD;
