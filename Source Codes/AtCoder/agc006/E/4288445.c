...
ull solve () {
	...
	ull arev = 0, brev = 0;
	for (i = 0; i < m; i++) {
		if (table[1][i] % 3 != 2) goto fail;
		if ((table[1][i] / 3) % 2 != i % 2) goto fail;
		if (table[0][i] == table[1][i] - 1 && table[2][i] == table[1][i] + 1) {
			if (i % 2 == 0) {
				a[alen] = table[1][i];
				alen++;
			} else {
		...
		} else if (table[0][i] == table[1][i] + 1 && table[2][i] == table[1][i] - 1) {
			if (i % 2 == 0) {
				a[alen] = table[1][i];
				alen++;
				arev++;
