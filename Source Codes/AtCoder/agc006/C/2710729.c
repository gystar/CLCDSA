...
ull solve () {
	...
	for (i = 0; i < m; i++) {
		permu[b[i]] ^= (permu[b[i] - 1] ^= (permu[b[i]] ^= permu[b[i] - 1]));
		// printf("%lld: ", i);
		// for (j = 0; j < n - 1; j++) printf("%lld ", permu[j]);
		// puts("");
	}
	...
	while (k) {
		...
		k /= 2;
		// for (i = 0; i < n - 1; i++) printf("%llu ", pres[i]);
		// 	printf("- ");
		// for (i = 0; i < n - 1; i++) printf("%llu ", permu[i]);
		// 	puts("");
	}
