...
bool isGeq (sll x) {
	...
	for (i = 0; i < n - 1; i++) {
		ull leftindex = (n - 1) - i;
		ull rightindex = (n - 1) + i;

		if (b[leftindex - 1] == b[leftindex]) {
			return (bool)b[leftindex];
		}
		if (b[rightindex] == b[rightindex + 1]) {
			return (bool)b[rightindex];
		}
	}
...
ull solve () {
	...
	result = nibutanlobo(isGeq, 1, n * 2);
