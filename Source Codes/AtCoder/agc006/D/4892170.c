...
int calc(int arr[]){
    int i,lp=-1,rp=-1;
    for(i=n;i>=2;i--){
        if(arr[i-1]==arr[i]){lp=i;break;}
    }
    for(i=n;i<=2*n-2;i++){
        if(arr[i]==arr[i+1]){rp=i;break;}
    }
    if(lp==-1&&rp==-1){
        return (arr[n]+(n-1))%2;
    }
    if(lp==-1){return arr[rp];}
    if(rp==-1){return arr[lp];}
    if(arr[rp]==arr[lp]){return arr[rp];}
    if((rp-n)<(n-lp)){return arr[rp];}
    else{return arr[lp];}
}

int main(void){
    int i,j,m,k,a[524288],b[524288],c,h,w,r=0,l,t;
    int st,fi,te;
    scanf("%d",&n);
    ...
    while(st<=fi){
        te=(st+fi)/2;
        for(i=1;i<=2*n-1;i++){
            if(a[i]<=te){b[i]=1;}else{b[i]=0;}
        }
        w=calc(b);
        if(w==1){fi=te-1;}else{st=te+1;}
    }
    printf("%d\n",st);
    return 0;
