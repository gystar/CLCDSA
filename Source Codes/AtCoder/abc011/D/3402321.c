...
double spdbnCr(double a,double b){
  long long i;
  double r=1.0f;
  for(i=1;i<=b;i++){r*=(a+1-i);r/=i;r*=0.25f;}
  return r;
}


int main(void){
    long long i,j,n,m,k,a[524288],b,c,h,w,l,t,d,x,y;
    long long p,q,r;
    double np,sp=0.0f;
    char s[524288];
    scanf("%lld%lld%lld%lld",&n,&d,&x,&y);
    w=d;
    if(x!=0){w=llgcd(w,llzt(0,x));}
    if(y!=0){w=llgcd(w,llzt(0,y));}
    d/=w;
    x/=w;
    y/=w;
    if(d!=1){
    ...
    for(i=0;i<=n;i++){
      ...
      if((i+p+q+r)>n){continue;}
      t=n-(i+p+q+r);
      if(t%2==1){continue;}
      q+=(t/2);
      r+=(t/2);
      np=1.0f;
      np*=spdbnCr((i+p+q+r),i);
      np*=spdbnCr((p+q+r),p);
      np*=spdbnCr((q+r),q);
      np*=spdbnCr((r),r);
      sp+=np;
