...
double calc(double x,double r,double h,double st,double fi){
    double hs,hf,v,p,q,t;
    hs=dbmax(x,st);
    hf=dbmin(x+h,fi);
    if(hs>=hf){return 0;}
    v=(r*r*pi*h)/3.0f;
...
int main(void){
    int i,j,n,m,k,c,w,l,t,q;
    double x[128],r[128],h[128],a,b;
    double d;
    char s[524288];
    scanf("%d%d",&n,&q);
    //l=strlen(s);
