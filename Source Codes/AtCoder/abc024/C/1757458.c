...
int main(void){
    int i,j,n,d,k,l[16384],r[16384],s[128],t[128],np,h;
    scanf("%d%d%d",&n,&d,&k);
    for(i=1;i<=d;i++){scanf("%d%d",&l[i],&r[i]);}
    for(i=1;i<=k;i++){scanf("%d%d",&s[i],&t[i]);}
    for(i=1;i<=k;i++){
        np=s[i];
        if(s[i] < t[i]){h=1;}else{h=-1;}
        for(j=1;j<=d;j++){
            if(l[j] <= np && np <= r[j]){
                if(h==1){np=r[j];}else{np=l[j];}
            }
            if((h==1&&t[i]<=np) || (h==-1&&np<=t[i])){printf("%d\n",j);break;}
        }
    }
    return 0;
