...
ull solve () {
	...
	for (i = 0; i < m; i++) {
		brr[i].a = brr[i].b = arr[i].a;
	}

	for (i = 0; i < k; i++) {
		for (j = 0; j < m; j++) {
			if (ansdata[j]) continue;
			if (brr[j].b < a[i] || brr[j].a > b[i]) continue;
			brr[j].a = umin(brr[j].a, a[i]);
			brr[j].b = umax(brr[j].b, b[i]);

			if (brr[j].a <= arr[j].b && arr[j].b <= brr[j].b) {
				ansdata[j] = i + 1;
			}
			// printf("%lld: %llu-%llu\n", j, brr[j].a, brr[j].b);
		}
