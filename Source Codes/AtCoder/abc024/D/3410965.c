...
int main(void){
    long long a,b,c,d,e,f,g,h,n,r;
    scanf("%lld%lld%lld",&a,&b,&c);
    d=a*modular_inverse(b);d%=mod;
    e=a*modular_inverse(c);e%=mod;
    f=b*modular_inverse(c);f%=mod;
    e=(mod+1-e)%mod;
    f=(1+f)%mod;
    g=e*modular_inverse(d);g%=mod;
    h=d*f;h%=mod;
    n=(mod+2-h)%mod;
    n*=modular_inverse((mod+h-1)%mod);n%=mod;
    r=(mod-g);
    r*=modular_inverse((mod+g-1)%mod);r%=mod;
    printf("%lld %lld\n",(mod+n-r)%mod,r);
    return 0;
