...
long long czt(long long lim,long long a,long long b){
  if(lim<a){return 0;}
  return 1+((lim-a)/b);
}

long long sigma(long long lim,long long a,long long b){
  if(lim<a){return 0;}  
  long long c=1+((lim-a)/b);
  long long p,q;
  p=a+(a+(c-1)*b);
  q=c;
  if(p%2==0){p/=2;}else{q/=2;}
  return p*q;
}

int main(void){
    long long i,j,n,m,k,a[524288],b[524288],c,h,w,r=0,l,t;
    long long st=0,fi=6666666666666666666,te;
    double d;
    char s[524288];
    scanf("%lld%lld",&k,&n);
    for(i=0;i<n;i++){
