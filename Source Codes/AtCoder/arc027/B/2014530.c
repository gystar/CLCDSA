...
ull seg_root (ull x) {
	...
	return parent[x] = seg_root(parent[x]);
}

ull seg_union (ull a, ull b) {
	ull ap = seg_root(a);
	ull bp = seg_root(b);
	if (ap == bp) return ap;
...
ull solve () {
	...
	seg_init(36);
	for (i = 0; i < n; i++) {
		seg_union(numnize(s[i]), numnize(t[i]));
	}

	// sum = 0;
	// ull s1root = seg_root(numnize(s[0]));
	// for (j = 0; j < 10; j++) {
	// 	if (seg_root(j) == s1root) break;
	// }
	// if (j == 10) {
	// 	ull s2root = seg_root(numnize(t[0]));
	// 	for (j = 0; j < 10; j++) {
	// 		if (seg_root(j) == s2root) break;
	// 	}

	// 	if (j == 10) {
	// 		for (j = 0; j < n; j++) {
	// 			if (s2root == seg_root(numnize(s[i]))) break;
	// 		}
	// 		if (j == n) {
	// 			sum = 2;
	// 		} else {
	...
	result = 1;
	for (i = 0; i < n; i++) {
		ull sroot = seg_root(numnize(s[i]));
		for (j = 0; j < 10; j++) {
			if (seg_root(j) == sroot) break;
		}
		if (j < 10) continue;

		ull multi = 10;
		if (seg_root(numnize(s[0])) == sroot || seg_root(numnize(t[0])) == sroot) multi = 9;
		result *= multi;
		seg_union(sroot, 0);
	}
