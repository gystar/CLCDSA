...
ull solve () {
	...
	for (i = 0; i < n; i++) {
		double origindist, dist;

		for (j = 0; j < n; j++) {
			origindist = distance(a[i], b[i], a[j], b[j]);
			dist = origindist - (double)(c[i] + c[j]);
			wall[i][j] = ((dist > 0) ? dist : 0);
		}

		origindist = distance(a[i], b[i], vsa, vsb);
		dist = origindist - (double)c[i];
		dp[i] = ((dist > 0) ? dist : 0);

		origindist = distance(a[i], b[i], vsc, vsd);
		dist = origindist - (double)c[i];
		dq[i] = ((dist > 0) ? dist : 0);
	}
	...
	for (ki = 0; ki < n; ki++) {
		for (i = 0; i < n; i++) {
			for (j = 0; j < n; j++) {
				double maybe = wall[i][ki] + wall[ki][j];
				if (maybe < wall[i][j]) wall[i][j] = maybe;
			}
		}
	}

	result = distance(vsa, vsb, vsc, vsd);

	for (i = 0; i < n; i++) {
		for (j = 0; j < n; j++) {
			double maybe = dp[i] + wall[i][j] + dq[j];
