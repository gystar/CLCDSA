...
int moving(int to,int kz){
    if(kz<=r){return zt(900,to);}
    if(kz<=g){return zt(1000,to);}
    if(kz<=b){return zt(1100,to);}
}
...
int main(void){
    ...
    for(i=0;i<2047;i++){
        for(j=b;j>0;j--){
            dp[i+1][j]=min(dp[i][j],dp[i][j-1]+moving(i,j));
        }
    }
    printf("%d\n",dp[2047][b]);
    return 0;
