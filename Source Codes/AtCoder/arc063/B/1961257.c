...
ull solve () {
	...
	for (i = 0; i < n - 1; i++) {
		// printf("%llu - %llu\n", dp[i + 1], dr[i]);
		if (dp[i + 1] < dr[i]) continue;
		gap = umax(gap, dp[i + 1] - dr[i]);
	}
	...
	while (i < n - 1) {
		if (dp[i + 1] - dr[i] == gap) {
			...
			while (maxlast - 1 >= 0 && dp[maxlast - 1] == dp[maxlast]) maxlast--;
			while (minlast + 1 < n && dr[minlast + 1] == dr[minlast]) minlast++;

			// printf("maxl: %lld, minl: %lld\n", maxlast, minlast);

			item = umin(dq[maxlast], ds[minlast]);
			result += item;
			// printf("res: %llu\n", result);
