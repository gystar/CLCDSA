...
int main(void){
    ...
    double st,fi,te,ars,arf,brs,brf;
    scanf("%d",&n);
    for(i=0;i<n;i++){
    ...
    for(i=0;i<1000;i++){
      ...
      brf=llinf;
      for(j=0;j<n;j++){
        ars=dbmax(ars,a[j]-(te/(double)c[j]));
        arf=dbmin(arf,a[j]+(te/(double)c[j]));
        brs=dbmax(brs,b[j]-(te/(double)c[j]));
        brf=dbmin(brf,b[j]+(te/(double)c[j]));        
      }
      if(ars<=arf && brs<=brf){fi=te;}
      else{st=te;}
      if(st>fi){break;}
    }
    printf("%.12lf\n",fi);
