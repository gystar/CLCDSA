...
bool func (double x) {
	...
	for (sll i = 0; i < n; i++) {
		double rad = x / (double)c[i];
		double maybe;

		maybe = a[i] - rad;
		if (!i || maybe > xmin) xmin = maybe;
		maybe = a[i] + rad;
		if (!i || maybe < xmax) xmax = maybe;
		maybe = b[i] - rad;
		if (!i || maybe > ymin) ymin = maybe;
		maybe = b[i] + rad;
		if (!i || maybe < ymax) ymax = maybe;

		if (xmin > xmax + EPS || ymin > ymax + EPS) return false;
	}
	return true;
}
