...
ull solve () {
	...
	m = 0;
	while (m <= n) {
		sll required = (n - m) * vue + 1 - k;

		if (required <= (sll)(vub * m)) {
			ull item;
			if (required <= (sll)(m * vud)) {
				item = 0;
			} else {
				item = (required - m * vud - 1) / (vub - vud) + 1;
			}
			ull maybe = vua * item + vuc * (m - item);
...
int32_t main (void) {
	...
	scanf("%llu%llu", &n, &k);
	// scanf("%llu", &k, &n);
	scanf("%llu%llu%llu%llu%llu", &vua, &vub, &vuc, &vud, &vue);
