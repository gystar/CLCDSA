...
long long ismev(long long x1,long long y1,long long x2,long long y2,long long x3,long long y3){
    long long d;
    x2-=x1;x3-=x1;
    y2-=y1;y3-=y1;
    d=llzt(x2*y3,x3*y2);
...
int main(void){
    long long i,j,k,n,m,t,x[128],y[128],r=0,l;
    double d;
    char s[262144];
    scanf("%lld",&n);
    //l=strlen(s);
    ...
    for(i=1;i<=n;i++){
        for(j=i+1;j<=n;j++){
            for(k=j+1;k<=n;k++){r+=ismev(x[i],y[i],x[j],y[j],x[k],y[k]);}
        }
    }
    printf("%lld\n",r);
    return 0;
