...
int main(void){
    ...
    for(i=1;i<=2*n;i++){
        for(j=0;j<=n+1;j++){
            if(s[i][0]=='B'){
                if(a[i]>=j){brw[i][j]=brw[i-1][j]+1;}
                else{brw[i][j]=brw[i-1][j];}
                wrw[i][j]=wrw[i-1][j];
            }
            else{
                if(a[i]>=j){wrw[i][j]=wrw[i-1][j]+1;}
                else{wrw[i][j]=wrw[i-1][j];}
                brw[i][j]=brw[i-1][j];
            }
        }
    }
    ...
    for(i=0;i<=n;i++){
        for(j=0;j<=n;j++){
            if(i!=0){
                dp[i][j]=min(dp[i][j],dp[i-1][j]+brw[bp[i]][i+1]+wrw[bp[i]][j+1]);
            }
            if(j!=0){
                dp[i][j]=min(dp[i][j],dp[i][j-1]+brw[wp[j]][i+1]+wrw[wp[j]][j+1]);
            }
            //printf("%d ",dp[i][j]);
        }//printf("\n");
    }
