...
bool dfs (sll i, ull len, ull k) {
	...
	for (c = 'a'; c <= 'z'; c++) {
		for (j = i; j < n; j++) {
			if (s[j] == c) {
				res[len] = c;
				if (dfs(j + 1, len + 1, k)) return true;
			}
		}
	}
...
int f (const void *left, const void *right) {
	hwll l = *(hwll*)left;
	hwll r = *(hwll*)right;

	int m = strncmp(s + l.a, s + r.a, umin(l.b, r.b));
	if (m) return m;
...
ull solve () {
	...
	qsort(x, u, sizeof(hwll), f);

	i = 0;
	k = q;
	while (--k) {
		j = i;
		while (j < u && x[j].b == x[i].b && strncmp(s + x[i].a, s + x[j].a, x[i].b) == 0) j++;
		i = j;
	}
