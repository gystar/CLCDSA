...
int main(void){
    long long i,j,n,t[1024],a[1024],nt,na;
    long long kt1,ka1,kt2,ka2;
    long long k1,k2;
    scanf("%lld",&n);
    for(i=1;i<=n;i++){
        scanf("%lld%lld",&t[i],&a[i]);
        if(i==1){nt=t[i];na=a[i];}
        else{
            k1=llceil(nt,t[i]);
            kt1=t[i]*k1;
            ka1=a[i]*k1;
            if(kt1<nt || ka1<na){kt1=llinf;ka1=llinf;}
            k2=llceil(na,a[i]);
            kt2=t[i]*k2;
            ka2=a[i]*k2;
            if(kt2<nt || ka2<na){kt2=llinf;ka2=llinf;}
            nt=llmin(kt1,kt2);
            na=llmin(ka1,ka2);
        }
    }
