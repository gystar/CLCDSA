...
ull solve () {
	...
	for (i = 0; i < n; i++) {
		...
		ull maygetTaka, maygetAo;
		ull mayTaka, mayAo;

		maygetTaka = ((takahashi - 1) / a[i] + 1) * a[i];
		maygetAo = ((aoki - 1) / b[i] + 1) * b[i];

		mayTaka = (maygetAo / b[i]) * a[i];
		mayAo = (maygetTaka / a[i]) * b[i];

		if (mayTaka > maygetTaka) {
			takahashi = mayTaka;
			aoki = maygetAo;
		} else {
			takahashi = maygetTaka;
			aoki = mayAo;
		}

	}

	result = takahashi + aoki;
