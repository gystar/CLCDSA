...
void dinic_bfs (const graph *g, const int32_t src, const int32_t dst, int32_t * const level, int32_t * const queue, const int32_t rev) {
  memset (level, 0, sizeof (int32_t) * g->vertex_num);
  level[src] = 1;
  int32_t front = 0;
  int32_t last = 0;
  queue[last++] = src;
  while (front < last && level[dst] == 0) {
    ...
    for (int32_t p = g->start[v]; p != -1; p = g->edge[p].next) {
      const int32_t u = g->edge[p].vertex;
      if (g->edge[p^rev].capacity > 0 && level[u] == 0) {
	level[u] = level[v] + 1;
	queue[last++] = u;
      }
    }
...
flow_type dinic (graph * const g, const int32_t src, const int32_t dst) {
  ...
  int32_t * const rev_level = (int32_t *) calloc (vertex_num, sizeof (int32_t));
  int32_t * const iter  = (int32_t *) calloc (vertex_num, sizeof (int32_t));
  flow_type flow = 0;
  while (1) {
    dinic_bfs (g, src, dst, level, queue, 0);
    if (level[dst] == 0) break;
    dinic_bfs (g, dst, src, rev_level, queue, 1);
    for (int32_t i = 0; i < vertex_num; ++i) {
      if (level[i] + rev_level[i] != level[dst] + 1) {
	level[i] = 0;
      }
    }
