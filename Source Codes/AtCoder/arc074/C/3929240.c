...
long long n,m;
long long dp[302][302][302]={0},r;

void trans(long long np,long long a,long long b,long long c,rs g[],mkj x[]){
  long long i,cnt;
  for(i=x[np].st;i<x[np].st+x[np].kz;i++){
    cnt=0;
    if(g[i].fi<=a){cnt++;}
    if(g[i].fi<=b){cnt++;}
    if(g[i].fi<=c){cnt++;}
    if(g[i].kr!=cnt){dp[a][b][c]=0;return;}
  }
  r+=dp[a][b][c];
  r%=mod;
  np++;
