...
int main(void){
    long long i,j,n,m,k,a,b,c=0,h,w,r=0,l,t;
    long long res[16]={0};
    sd dat[1048576];
    scanf("%lld%lld%lld",&h,&w,&n);
    for(i=0;i<n;i++){
      scanf("%lld%lld",&a,&b);
      for(j=llmax(1,a-2);j<=llmin(a,h-2);j++){
        for(k=llmax(1,b-2);k<=llmin(b,w-2);k++){
          dat[c].av=j;
          dat[c].bv=k;
          c++;
        }
    ...
    t=1;
    for(i=1;i<=c;i++){
      if(i==c || dat[i].av!=dat[i-1].av || dat[i].bv!=dat[i-1].bv){
        res[t]++;
        t=1;
      }else{
        t++;
