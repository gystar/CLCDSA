...
long long mfac[16384];

void calc(){
    mfac[0]=1;mfac[1]=1;
    long long i;
    for(i = 2;i < 16384;i++){mfac[i]=mfac[i-1]*i;mfac[i]%=mod;}
}

int main(void){
    calc();
    long long i,j,n,k,l,a[131072],b[131072]={0},rw=0,r=0;
    scanf("%lld",&n);
    for(i = 0;i < n;i++){scanf("%lld",&a[i]);b[a[i]]++;}
    qsort(a,n,sizeof(long long),sortfncsj);
    for(i = 0;i < n;i++){rw+=a[i];r+=rw;}
    printf("%lld\n",r);
    r=1;
    for(i = 1;i < 16383;i++){r*=mfac[b[i]];r%=mod;}
    printf("%lld\n",r);
    return 0;
