...
void arrinit(long long a[],long long b[],long long n){
  long long i;
  for(i=0;i<n;i++){
    a[i]=b[i];
    b[i]=0;
...
int main(void){
    long long i,j,n,m,k,a,b,c,h,w,r=0,l,t,f;
    long long dp[524288]={0},ndp[524288]={0};
    scanf("%lld%lld%lld%lld",&n,&a,&b,&c);
    w=(1ll<<(a+b+c));
    f=((1ll<<(a+b+c-1))|(1ll<<(b+c-1))|(1ll<<(c-1)));
    dp[0]=1;
    for(i=0;i<n;i++){
      for(j=0;j<w;j++){
        l=j;
