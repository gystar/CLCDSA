...
ull solve () {
	...
	ull fuck = (1 << (vua + vub + vuc)) + (1 << (vub + vuc)) + (1 << vuc) + 1;
	ull bitlen = (vua + vub + vuc + 1);
	ull ablemask = (1 << bitlen) - 1;
	...
	for (i = 0; i < n; i++) {
		...
		for (j = 1; !(j >> bitlen); j++) {
			if (!bitdp[curri][j]) continue;
			
			for (ki = 1; ki <= 10; ki++) {
				dpcell = &bitdp[nexti][((j << ki) & ablemask) + 1];
				*dpcell = (*dpcell + bitdp[curri][j]) % MOD;
			}
		}
	...
	result = 0;
	for (j = 0; !(j >> bitlen); j++) {
		result = (result + bitdp[n & 1][j]) % MOD;
	}

	result = (bitpow(10, n, MOD) + MOD - result) % MOD;
