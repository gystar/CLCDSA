...
ull solve () {
	...
	dp[0][0] = dp[0][1] = 1;
	dpsum[0][0] = dpsum[0][1] = 1;
	for (i = 0; i < n; i++) {
		for (j = 0; j < 2; j++) {
			sll lower = x[i][j] - 1;
			sll upper = smin(i - 1, y[i + 1][j ^ 1]);

			if (lower > upper) {
				dp[i + 1][j] = 0;
			} else {
				dp[i + 1][j] = (MOD + dpsum[upper + 1][j ^ 1] - (lower >= 0 ? dpsum[lower][j ^ 1] : 0)) % MOD;
			}
			// printf("%llu ", dp[i + 1][j]);

			dpsum[i + 1][j] = (dpsum[i][j] + dp[i + 1][j]) % MOD;
		}
		// puts("");
	}
