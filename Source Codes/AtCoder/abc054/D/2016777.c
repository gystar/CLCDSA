...
ull leftall[M_MAX + 1][M_MAX + 1];
ull rightall[M_MAX + 1][M_MAX + 1];
...
ull solve () {
	...
	for (i = 0; i < leftn; i++) leftsum += a[i];
	for (j = 0; j < rightn; j++) rightsum += a[leftn + j];

	for (i = 0; i <= M_MAX; i++) {
		for (j = 0; j <= M_MAX; j++) {
			leftall[i][j] = rightall[i][j] = BIG;
		}
	}
	leftall[0][0] = rightall[0][0] = 0;

	for (i = 0; i < (1 << leftn); i++) {
		ull asum = 0, bsum = 0, csum = 0;
		for (j = 0; j < leftn; j++) {
			if (i & (1 << j)) {
				asum += a[j];
				bsum += b[j];
		...
		dpcell = &leftall[asum][bsum];
		*dpcell = umin(*dpcell, csum);
	}

	for (i = 0; i < (1 << rightn); i++) {
		ull asum = 0, bsum = 0, csum = 0;
		for (j = 0; j < rightn; j++) {
			if (i & (1 << j)) {
				asum += a[leftn + j];
	...
	for (i = 0; i <= leftsum; i++) {
		...
		for (j = 0; j <= rightsum; j++) {
			// if (j % 100 == 0) printf("[%llu %llu]\n", i, j);

			ull asum, bsum;

			asum = i;
			while (asum <= leftsum + rightsum) {
				if (((asum * vub) % vua) || !asum) {
					asum++;
					continue;
				}
				bsum = asum * vub / vua;
				if (asum > M_MAX || bsum > M_MAX) break;
				if (bsum >= j) {

					maybe = leftall[i][j] + rightall[asum - i][bsum - j];
					// printf("[%llu + %llu][%llu + %llu] %llu\n", i, (asum - i), j, (bsum - j), maybe);
					if (maybe < result) result = maybe;
				}

				asum += vua / gcd(vua, vub);
			}
			// printf("[%llu][%lld] %llu\n", i, j, maybe);
		}
	}
