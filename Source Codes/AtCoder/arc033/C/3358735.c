...
AVLNode* newAVLNode(void *v,size_t size,AVLTree *t){
  AVLNode *res;
  if(t->freeNodes!=NULL){
    res=t->freeNodes;
    t->freeNodes=t->freeNodes->right;
  } else {
    res=(AVLNode *)malloc(sizeof(AVLNode));
  }
  res->val=malloc(size);
...
void freeAVLNode(AVLNode *r,AVLTree *t){
  r->right=t->freeNodes;
  t->freeNodes=r;
  return;
}
...
AVLNode* insert_func(AVLNode *r,void *v,AVLTree *t){
  if(r==NULL) return newAVLNode(v,t->valSize,t);
  if(t->cmp(r->val,v)>=0){
