...
int main(void){
    int i,j,n,m,t,k,a[262144],b[262144],r=0,l;
    int all=0,exi=0;
    double d;
    char s[262144];
    scanf("%d%d",&n,&m);
    //l=strlen(s);
    for(i=0;i<n;i++){scanf("%d",&a[i]);}a[n]=inf;
    for(i=0;i<m;i++){scanf("%d",&b[i]);}b[m]=inf;
    qsort(a,n,sizeof(int),sortfncsj);
    qsort(b,m,sizeof(int),sortfncsj);
    i=0;j=0;
    while(i<n || j<m){
        //printf("<%d %d %d %d>",a[i],b[j],all,exi);
        if(a[i]==b[j]){all++;exi++;i++;j++;}
        else if(a[i]<b[j]){exi++;i++;}
        else{exi++;j++;}
    }
    d=all;d/=exi;
    printf("%.12lf\n",d);
