...
ull roadRank[N_MAX];
ull roadParent[N_MAX];
ull subwayRank[N_MAX];
ull subwayParent[N_MAX];

ull uf_init (ull n) {
	sll i;
	for (i = 0; i < n; i++) {
		roadRank[i] = subwayRank[i] = 0;
		roadParent[i] = subwayParent[i] = i;
	}
	return n;
}

ull uf_find (ull x, ull parents[]) {
	if (parents[x] == x) return x;
	return parents[x] = uf_find(parents[x], parents);
}

ull uf_union (ull a, ull b, ull parents[], ull ranks[]) {
	ull ap = uf_find(a, parents);
	ull bp = uf_find(b, parents);
	if (ap == bp) return ap;
...
ull solve () {
	...
	for (i = 0; i < m; i++) {
		uf_union(arr[i].a, arr[i].b, roadParent, roadRank);
	}
	for (i = 0; i < k; i++) {
		uf_union(brr[i].a, brr[i].b, subwayParent, subwayRank);
	}

	for (i = 0; i < n; i++) {
		ull hash = uf_find(i, roadParent) * n + uf_find(i, subwayParent);
		dp[i].a = hash;
		dp[i].b = i;
	}
	qsort(dp, n, sizeof(hwll), phwllABcomp);

	i = 0;
	while (i < n) {
		j = i;
		while (j < n && dp[j + 1].a == dp[i].a) j++;
		for (ki = i; ki <= j; ki++) {
			dq[dp[ki].b] = (j - i + 1);
		}
		i = j + 1;
...
int32_t main (void) {
	...
	// scanf("%*llu");
	scanf("%llu", &k, &m, &n);
	// scanf("%llu%llu%llu%llu%llu", &vua, &vub, &vuc, &vud, &vue, &vuf);
