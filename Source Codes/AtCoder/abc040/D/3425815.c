...
if(((query*)a)->year > ((query*)b)->year){return -1;}
if(((query*)a)->year < ((query*)b)->year){return 1;}
if(((query*)a)->type < ((query*)b)->type){return -1;}
if(((query*)a)->type > ((query*)b)->type){return 1;}
return 0;
}
...
int main(void){
    resuf();
    int i,j,n,m,k,a,b,y,c,h,v,w,r=0,l,t,q;
    int res[524288]={0};
    double d;
    char s[524288];
    query qs[524288];
    ...
    qsort(qs,m+q,sizeof(qs[0]),sortfnc);
    for(i=0;i<m+q;i++){
      if(qs[i].type==1){
        uni(qs[i].fd,qs[i].sd);
      }
      else{
        res[qs[i].sd]=size(qs[i].fd);
      }
    }
