...
ull solve () {
	...
	qsort(dp, (1 << leftn), sizeof(ull), pullcomp);
	qsort(dq, (1 << rightn), sizeof(ull), pullcomp);
	...
	for (i = 0; i < (1 << leftn); i++) {
		...
		resfordpi = 0;
		ull required = k - dp[i];
		while (j >= 0 && dq[j] > required) j--;
		if (j < 0) break;

		ki = j;
		while (j >= 0 && dq[j] == required) {
			resfordpi++;
			j--;
		}

		result += resfordpi;
		// printf("i: %lld, dp[i]: %llu, j: %lld, resfordpi: %llu, result: %llu\n", i, dp[i], j, resfordpi, result);
	}
