...
int main(void){
    resheap();
    long long i,j,n,m,c[131072],a,b,r,e=0,nv;
    rs g[1048576];
    mkj x[131072];
    bool fl[131072]={0};
    data pd,od;
    ...
    for(i=1;i<=n;i++){
      scanf("%lld",&c[i]);
      g[e].st=0;g[e].fi=i;g[e].kr=c[i];e++;
    }
    for(i=1;i<=m;i++){
      scanf("%lld%lld%lld",&a,&b,&r);
      g[e].st=a;g[e].fi=b;g[e].kr=r;e++;
      g[e].st=b;g[e].fi=a;g[e].kr=r;e++;
    }
    qsort(g,e,sizeof(rs),sortfnc);
    makemkj(g,x,e);
    pd.node=0;
    pd.cost=0;
    pqpush(pd);
