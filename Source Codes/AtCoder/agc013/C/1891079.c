...
ull solve () {
	...
	ull standard = dp[0];
	for (i = 0; i < n; i++) {
		dp[i] = (vua + dp[i] - standard) % vua;
	}
	qsort(dp, n, sizeof(ull), pullcomp);
	for (i = 0; i < n; i++) {
		dp[i] = (dp[i] + standard) % vua;
	}
	...
	for (i = 0; i < n; i++) {
		...
		ull collig = (vub * 2 + (vua - dist) - (2 - arr[0].b)) / vua;
		// printf("%lld: %llu\n", i, collig);
		sum += collig;
	}
	sum %= n;
	...
	// if (dp[0] != dp[n - 1] && dp[(n - sum) % n] == dp[(n - sum + 1) % n]) {
	// 	ull s = 0;
	// 	for (i = 0; i < n; i++) {
	// 		if (arr[1].b == arr[i].b) continue;

	// 		ull dist;
	// 		ull clockwise = (vua + arr[i].a - arr[1].a) % vua;
	// 		if (arr[1].b == 1) {
	// 			dist = clockwise;
	// 		} else {
	// 			dist = (vua - clockwise) % vua;
	...
	// 		ull col = (vub * 2 + (vua - dist)) / vua;
	// 		s += col;

	// 	}
	// 	s %= n;
	// 	if (arr[1].b != 1) sum = (n - sum) % n;
