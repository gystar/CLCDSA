...
int sdsortfnc(const void *a,const void *b){
  int i;
  for(i=0;i<26;i++){
    if(((sd*)a)->val[i] < ((sd*)b)->val[i]){return -1;}
    if(((sd*)a)->val[i] > ((sd*)b)->val[i]){return 1;}
  }
  return 0;
}
...
int main(void){
    int i,j,n,m,k,a[32]={0},b,c,h,w,r=0,l,t;
    int p,q;
    sd data[131072];
    double d;
    char s[524288];
    ...
    for(i=k;i<n;i++){
      data[i-k+1]=data[i-k];
      data[i-k+1].node=i-k+1;
      data[i-k+1].val[s[i-k]-'a']--;
      data[i-k+1].val[s[i]-'a']++;
    }
    qsort(data,n-k+1,sizeof(data[0]),sdsortfnc);
    p=data[0].node;q=data[0].node;
    for(i=1;i<n-k+1;i++){
      for(j=0;j<26;j++){
        if(data[i-1].val[j]!=data[i].val[j]){
          p=data[i].node;
          q=data[i].node;
          break;
        }
