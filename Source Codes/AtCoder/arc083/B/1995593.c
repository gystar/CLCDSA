...
bool isiwi (char x, char y, char z) {
	return (x == 'i' && y == 'w' && z == 'i');
}
...
ull solve () {
	...
	for (ki = 0; ki < n; ki++) {
		for (i = 0; i < n; i++) {
			for (j = 0; j < n; j++) {
				if (a[i][j] > a[i][ki] + a[ki][j]) goto fail;
			}
		}
	}
	...
	for (ki = 0; ki < n; ki++) {
		for (i = 0; i < n; i++) {
			...
			for (j = 0; j < n; j++) {
				...
				if (a[i][ki] == BIG || a[ki][j] == BIG || a[i][j] == BIG) continue;
				if (a[i][j] == a[i][ki] + a[ki][j]) a[i][j] = BIG;
			}
		}
	}
