...
ull solve () {
	...
	waterdp[0] = true;
	for (i = vua; i <= vuf / 100; i++) {
		if ((i >= vua && waterdp[i - vua]) || (i >= vub && waterdp[i - vub])) {
			waterdp[i] = true;
		}
	}
	...
	sugardp[0] = true;
	for (i = vuc; i <= vuf; i++) {
		if ((i >= vuc && sugardp[i - vuc]) || (i >= vud && sugardp[i - vud])) {
			sugardp[i] = true;
		}
	}
	...
	for (i = vua; i <= vuf / 100; i++) {
		if (!waterdp[i]) continue;
		for (j = 0; j <= vuf - i * 100; j++) {
			if (j > vue * i) break;
			if (!sugardp[j]) continue;
