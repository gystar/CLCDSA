...
ull solve () {
	...
	for (i = 0; i <= n; i++) {
		...
		ncr[i] = divide((ncr[i - 1] * (n + 1 - i)) % m, i, m);
	}
	for (i = 0; i <= n * n; i++) {
		if (!i) {
			shif2[i] = 1;
	...
	result = bitpow(2, bitpow(2, n, m - 1), m);
	// printf(":%llu\n", result);
	for (i = 1; i <= n; i++) {
		const ull others = bitpow(2, bitpow(2, n - i, m - 1), m);

		sum = 0;
		for (j = 1; j <= i; j++) {
			item = (j * dp[i][j] % m) * shif2[(n - i) * (j - 1)] % m;
			sum = (sum + item) % m;

			item = dp[i][j] * shif2[(n - i) * j] % m;
			sum = (sum + item) % m;
		}
		sum = (sum * others) % m;
