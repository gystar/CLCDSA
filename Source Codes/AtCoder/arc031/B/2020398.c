...
void dfsmap (sll i, sll j, ull color) {
	if (s[i][j] != 'o') return;
	if (dp[i][j]) return;
	dp[i][j] = color;

	if (i > 0) dfsmap(i - 1, j, color);
	if (j > 0) dfsmap(i, j - 1, color);
	if (i < h - 1) dfsmap(i + 1, j, color);
	if (j < w - 1) dfsmap(i, j + 1, color);
...
ull solve () {
	...
	for (i = 0; i < h; i++) {
		for (j = 0; j < w; j++) {
			...
			if (i > 0) isfilled[dp[i - 1][j]]++;
			if (j > 0) isfilled[dp[i][j - 1]]++;
			if (i < h - 1) isfilled[dp[i + 1][j]]++;
			if (j < w - 1) isfilled[dp[i][j + 1]]++;

			bool isbridge = true;
			for (ki = 1; ki <= sum; ki++) {
				if (!isfilled[ki]) isbridge = false;
			}
			if (isbridge) goto success;
