...
void clbig(int ver,int pc,int col[],rs g[],mkj x[]){
  int i;
  if(col[ver]!=0){
    if(col[ver]!=(3-pc)){err=true;}
    return;
  }
  col[ver]=3-pc;
  if(col[ver]==1){oc++;}else{tc++;}
...
int main(void){
  rs g[524288];
  mkj x[1024];
  bool fl[1024][1024]={0},dp[1024]={0};
  int col[1024]={0};
  int i,j,n,m,k,a,b,c,r;
  scanf("%d%d",&n,&m);
  dp[0]=true;
  for(i=0;i<m;i++){
    scanf("%d%d",&a,&b);
  ...
  for(i=1;i<=n;i++){
    for(j=1;j<=n;j++){
      if(i==j || fl[i][j]){continue;}
      g[c].st=i;
      g[c].fi=j;
      c++;
    }
  }
  qsort(g,c,sizeof(g[0]),sortfnc);
  makemkj(g,x,c);
  for(i=1;i<=n;i++){
    if(col[i]!=0){continue;}
    err=false;
