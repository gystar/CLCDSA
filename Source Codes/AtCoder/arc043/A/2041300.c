...
ull solve () {
	...
	ull minval = BIG, maxval = 0;
	for (i = 0; i < n; i++) {
		minval = umin(minval, a[i]);
		maxval = umax(maxval, a[i]);
		sum += a[i];
	}
	...
	if (minval == maxval) {
		...
		double p = (double)(vub) / (double)(maxval - minval);
		double q = (double)vua - (double)(p * (double)sum) / (double)n;
		printf("%.12lf %.12lf\n", p, q);
	}
