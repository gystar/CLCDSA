...
int main(void){
    long long i,j,n,m,k,a[524288],b[524288]={0},c,w,r=0,l,t;
    long long frw[524288]={0},brw[524288]={0};
    long long sc[524288]={0},tc[524288]={0},trw[524288]={0};
    double d;
    char s[524288];
    scanf("%lld",&n);
    //l=strlen(s);
    for(i=0;i<n;i++){scanf("%lld",&a[i]);b[a[i]]++;}
    for(i=1;i<200000;i++){
        frw[i]=frw[i-1]+b[i];frw[i]%=mod;
    }
    for(i=200000;i>0;i--){
        brw[i]=brw[i+1]+b[i];brw[i]%=mod;
    }
    for(i=1;i<200000;i++){
        sc[i]=frw[i/2]*b[i];
    }
    ...
    for(i=200000;i>0;i--){
        trw[i]=trw[i+1]+tc[i];trw[i]%=mod;
    }
    for(i=1;i<200000;i++){
        r+=sc[i]*trw[i*2];r%=mod;
    }
    //shuffledget(a,n);
