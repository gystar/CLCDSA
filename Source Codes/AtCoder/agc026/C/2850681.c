...
long long search(char s[]){
    ...
    p=h;
    while(1){
        if(strcmp(hmap[p].str,s)==0){return p;}
        if(strlen(hmap[p].str)==0){return -1;}
        p++;p%=bas;
        if(p==h){return -1;}
    }
}
...
void add(char s[]){
    long long h,p;
    h=search(s);
    if(h!=-1){hmap[h].val++;return;}
    h=chash(s);
    p=h;
    while(1){
        if(strlen(hmap[p].str)==0){
            strcpy(hmap[p].str,s);
            hmap[p].val++;
            return;
        }
...
int main(void){
    ...
    long long mem[1048576]={0};
    double d;
    char s[524288],x[32],y[32],z[32];
    scanf("%lld%s",&n,s);
    //l=strlen(s);
    for(i=0;i<n;i++){x[i]=s[i];}
    for(i=0;i<n;i++){y[i]=s[2*n-1-i];}
    k=llpow(2,n);
    for(i=0;i<k;i++){
        p=0;
        t=i;
