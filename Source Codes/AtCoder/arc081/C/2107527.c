...
ull solve () {
	...
	for (i = slen - 1; i >= 0; i--) {
		for (j = 0; j < 26; j++) {
			...
			ull mincompleted = BIG;
			for (ki = 0; ki < 26; ki++) {
				mincompleted = umin(mincompleted, dp[i + 1][ki]);
			}

			dp[i][j] = mincompleted + 1;
		}
	}

	result = BIG;
	for (i = 0; i < 26; i++) {
		result = umin(result, dp[0][i]);
	}
	result++;
