...
ull hoge (sll i, sll j) {
	if (i == h - 1 || j == w - 1) return 1;
	return isb(i, j) + isb(i, j + 1) + isb(i + 1, j) + isb(i + 1, j + 1);
}

hwll stk[3001];
ull hist (ull hind, ull w) {
	ull stlen = 0;
	ull r = 0;
	for (sll i = 0; i < w; i++) {
		ull last = i;
		while (stlen && stk[stlen - 1].a > dq[hind][i]) {
			r = umax(r, (stk[stlen - 1].a + 1) * (i - stk[stlen - 1].b + 1));
			last = stk[stlen - 1].b;

			stlen--;
		}
		if (!stlen || stk[stlen - 1].a < dq[hind][i]) {
			stk[stlen].a = dq[hind][i];
			stk[stlen].b = last;
...
int32_t main (void) {
	...
	// scanf("%lld%lld%lld", &vsa, &vsb, &vsc, &vsd);
