...
void seg_init () {
	...
	seg_baseindex = 1;
	while (seg_baseindex < slen) seg_baseindex *= 2;

	for (i = 0; i < seg_baseindex; i++) {
		if (i < slen) {
			seg[seg_baseindex + i] = (s[i] == 'A' ? 1 : 2);
		} else {
			seg[seg_baseindex + i] = 0;
		}
	}

	for (i = seg_baseindex - 1; i >= 1; i--) {
		seg[i] = (seg[i * 2] + seg[i * 2 + 1]);
	}

	teg_baseindex = 1;
	while (teg_baseindex < tlen) teg_baseindex *= 2;

	for (i = 0; i < teg_baseindex; i++) {
		if (i < tlen) {
			teg[teg_baseindex + i] = (t[i] == 'A' ? 1 : 2);
		} else {
			teg[teg_baseindex + i] = 0;
		}
	}

	for (i = teg_baseindex - 1; i >= 1; i--) {
		teg[i] = (teg[i * 2] + teg[i * 2 + 1]);
	}
}

int seg_sum (int seg[], ull segbase, int index /* 1-indexed */) {
	ull segb = segbase;
	int result = 0;

	while (index) {
		if (index & 1) {
			result += seg[segb + index - 1];
		}
		segb /= 2;
		index /= 2;
...
ull solve () {
	...
	for (i = 0; i < n; i++) {
		...
		int shash = seg_sum(seg, seg_baseindex, vub) - seg_sum(seg, seg_baseindex, vua - 1);
		int thash = seg_sum(teg, teg_baseindex, vud) - seg_sum(teg, teg_baseindex, vuc - 1);
