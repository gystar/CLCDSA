...
int main(void){
    long long i,n,m,x[262144],y[262144];
    long long xs=0,ys=0,k;
    scanf("%lld%lld",&n,&m);
    scanf("%lld",&x[1]);
    for(i=2;i<=n;i++){scanf("%lld",&x[i]);
        k=(i-1)*(n+1-i);k%=mod;
        xs+=(x[i]-x[i-1])*k;xs%=mod;
    }
    scanf("%lld",&y[1]);
    for(i=2;i<=m;i++){scanf("%lld",&y[i]);
        k=(i-1)*(m+1-i);k%=mod;
        ys+=(y[i]-y[i-1])*k;ys%=mod;
    }
    printf("%lld\n",(xs*ys)%mod);
    return 0;
