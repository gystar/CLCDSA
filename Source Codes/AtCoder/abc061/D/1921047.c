...
ull solve () {
	...
	for (i = 0; i < n; i++) {
		d[i] = (i ? VERYBIG : 0);
		revd[i] = ((i != n - 1) ? VERYBIG : 0);
	}

	for (i = 0; i < n; i++) {
		for (j = 0; j < m; j++) {
			if (revd[b[j]] != VERYBIG && (revd[a[j]] == VERYBIG || revd[a[j]] > revd[b[j]] + c[j])) {
				revd[a[j]] = revd[b[j]] + c[j];
			}
		}
	}
	...
	for (i = 0; i < n; i++) {
		for (j = 0; j < m; j++) {
			if (d[a[j]] != VERYBIG && (d[b[j]] == VERYBIG || d[b[j]] > d[a[j]] + c[j])) {
				d[b[j]] = d[a[j]] + c[j];
				if (i == n - 1 && revd[b[j]] != VERYBIG) goto fail;
			}
		}
	}
