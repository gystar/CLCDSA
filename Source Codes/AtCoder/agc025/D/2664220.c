...
void fill (ull d, ull index) {
	...
	for (sll i = 0; i < n * 2; i++) {
		for (sll j = 0; j < n * 2; j++) {
			dx[index][i][j] = map[(i / pow4) % 2][(j / pow4) % 2];
		}
	}
}
...
ull solve () {
	...
	for (i = 0; i < n * 2; i++) {
		for (j = 0; j < n * 2; j++) {
			dnum[dx[0][i][j] * 2 + dx[1][i][j]]++;
		}
	}
	ull index;
	for (index = 0; ; index++) {
		if (dnum[index] >= n * n) break;
	}

	ull len = 0;
	for (i = 0; i < n * 2; i++) {
		for (j = 0; len < n * n && j < n * 2; j++) {
			if (dx[0][i][j] * 2 + dx[1][i][j] == index) {
				printf("%llu %llu\n", i, j);
				len++;
			}
		}
