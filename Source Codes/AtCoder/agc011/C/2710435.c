...
ull color[N_MAX];
bool dfs (ull v, ull currcol) {
	bool isbwable = true;

	color[v] = currcol;
	for (sll i = glen[v]; i < glen[v + 1]; i++) {
		ull u = g[i].b;

		if (color[u]) {
			if (color[u] == currcol) isbwable = false;
		} else {
			isbwable &= dfs(u, currcol ^ 3);
		}
	}
...
ull solve () {
	...
	qsort(g, m * 2, sizeof(hwll), phwllABcomp);

	i = 0;
	j = 0;
	while (i < n) {
		glen[i] = j;
		while (j < m * 2 && g[j].a == i) j++;

		i++;
	}
	glen[n] = m * 2;

	ull alone = 0, bw = 0, notbw = 0;
	for (i = 0; i < n; i++) {
		if (color[i]) continue;
		if (glen[i] == glen[i + 1]) {
			alone++;
	...
	// printf("alone: %llu, bw: %llu, notbw: %llu\n", alone, bw, notbw);
	result = (n * n - (n - alone) * (n - alone)) + ((bw + notbw) * (bw + notbw) + bw * bw);
