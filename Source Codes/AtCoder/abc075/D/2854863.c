...
int main(void){
    long long i,j,n,m,k,a[64],b[64],x[64],y[64],c,h,w,r=llinf,l,t;
    long long xi,xj,yi,yj;
    double d;
    char s[524288];
    scanf("%lld%lld",&n,&k);
    //l=strlen(s);
        ...
        for(xj=xi+1;xj<n;xj++){
            for(yi=0;yi<n;yi++){
                for(yj=yi+1;yj<n;yj++){
                    t=0;
                    for(i=0;i<n;i++){
                        if(a[xi]<=x[i] && x[i]<=a[xj] && b[yi]<=y[i] && y[i]<=b[yj]){t++;}
                    }
                    if(t>=k){r=llmin(r,(a[xj]-a[xi])*(b[yj]-b[yi]));}
                }
            }
        }
    }
