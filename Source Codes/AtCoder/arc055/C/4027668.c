...
void z_algorithm(char s[],int arr[]){
  int i,j,k,l;
  l=strlen(s);
  arr[0]=l;
  i=1;j=0;
...
int main(void){
    ...
    long long res=0;
    int pf[size],rpf[size];
    char s[size]={0},revs[size]={0};
    scanf("%s",s);
    l=strlen(s);
    for(i=l-1;i>=0;i--){
      revs[l-1-i]=s[i];
    ...
    z_algorithm(s,pf);
    z_algorithm(revs,rpf);
    for(i=2;i<l-2;i++){
      //does B exist?
      if((i+1)<=(l-i-1)){continue;}
      a=min((l-i-2),pf[i+1]);
      c=min((l-i-2),rpf[l-i-1]);
      if(a==0 || c==0){continue;}
      if(a+c<(l-i-1)){continue;}
      //printf("%d %d %d %d %d\n",i,a,c,a+c-(l-i-1)+1,(l-i-1)-1);
      res+=(a+c-(l-i-1)+1);
    }
    printf("%lld\n",res);
    return 0;
