...
int main(void){
    long long n,a,b,c,d,sg,i;
    long long umax,umin,smax,smin;
    scanf("%lld%lld%lld%lld%lld",&n,&a,&b,&c,&d);
    sg = a-b;
    for(i=0;i<=(n-1);i++){
        umax = d*i;umin = c*i;
        smax = d*(n-i-1);smin = c*(n-i-1);
        if(umin-smax <= sg && sg <= umax-smin){
            printf("YES\n");return 0;
        }
    }
    printf("NO\n");
...
./Main.c:17:5: warning: conflicting types for built-in function ‘pow’
 int pow(int a,int b){int i,r=1;for(i=1;i<=b;i++){r*=a;}return r;}
     ^
./Main.c:20:11: warning: conflicting types for built-in function ‘llround’
 long long llround(long long a,long long b){if((a%b)*2 >= b){return (a/b)+1;}return a/b;}
           ^
