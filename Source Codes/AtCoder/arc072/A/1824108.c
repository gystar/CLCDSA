...
int main(void){
    long long i,j,n,k,a[262144];
    long long pd=0,md=0,x;
    scanf("%lld",&n);
    for(i=1;i<=n;i++){scanf("%lld",&a[i]);}
    x=0;
    for(i=1;i<=n;i++){
        x+=a[i];
        if(i%2==1 && x<=0){pd+=llzt(1,x);x=1;}
        if(i%2==0 && x>=0){pd+=llzt(-1,x);x=-1;}
    }
    x=0;
    for(i=1;i<=n;i++){
        x+=a[i];
        //printf("%lld\n",x);
        if(i%2==0 && x<=0){md+=llzt(1,x);x=1;}
        if(i%2==1 && x>=0){md+=llzt(-1,x);x=-1;}
    }
    //printf("%lld %lld\n",pd,md);
    printf("%lld\n",llmin(pd,md));
    return 0;
