...
int main(void){
    int n,t[128],v[128]={0},i,j,s=0,vm[65536];
    double sl[65536],nv=0.0f,sv,res=0.0f,vmm;
    scanf("%d",&n);
    for(i=1;i<=n;i++){
        scanf("%d",&t[i]);
        s+=(t[i]*2);t[i]=s;
    ...
    for(i=1;i<=n;i++){
        for(j=t[i-1]+1;j<=t[i];j++){
            vm[j]=v[i];
            if(j==t[i]){vm[j]=min(v[i],v[i+1]);}
        }
    }
    sl[s]=0.0f;
    for(i=s-1;i>=0;i--){
        vmm=vm[i];
        sl[i]=dbmin(sl[i+1]+0.5f,vmm);
    }
    for(i=1;i<=s;i++){
        sv=dbmin(nv+0.5f,sl[i]);
        res+=((nv+sv)*0.25f);
        //if(nv!=sv){printf("%d %lf ",i,sv);}
        nv=sv;
    }
    printf("%.12lf\n",res);
    return 0;
